...
class StateRepublisher(Node):
    ...
    def __init__(self):
        ...
        for axis in utils.get_axes():
            self._pub_pose[axis] = self.create_publisher(
                Float64, self._get_pose_topic(axis), 10
            )
            self._pub_pose[axis].publish(
                Float64(0)
            )  # not sure why there a need to publish 0 when initialising state republisher
            self._pub_twist[axis] = self.create_publisher(
                Float64, self._get_twist_topic(axis), 10
            )
            self._pub_twist[axis].publish(Float64(0))

        # Create local state publisher
        self._pub_local_state = self.create_publisher(
            Odometry, self.LOCAL_STATE_TOPIC, 10
        )
    ...
    def _receive_odometry(self, odometry):
        try:
            ...
            utils._publish_data_dictionary(
                self._pub_pose, utils._parse_pose(local_pose)
            )
            utils._publish_data_dictionary(
                self._pub_twist, utils._parse_twist(local_twist)
            )
