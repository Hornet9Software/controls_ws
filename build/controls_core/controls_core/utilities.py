...
def reached_pose(
    current_pose: Pose, desired_pose: Pose, linear_tolerance=0.1, angular_tolerance=3
) -> bool:
    # Compute linear distance between 2 points and check if it is within tolerance
    # Because we used vectors, sign does not matter!
    linear = (
        linear_distance(current_pose.position, desired_pose.position) < linear_tolerance
    )
    # Obtain RPY difference
    angular_dist = angular_distance_quat(
        current_pose.orientation, desired_pose.orientation
    )
    # Check if all of them are lesser than the tolerance
    angular = np.all(
        np.array([angular_dist.x, angular_dist.y, angular_dist.z])
        < (np.ones(3) * angular_tolerance)
    )
    return linear and angular


def at_desired_velocity(
    current_twist: Twist,
    desired_twist: Twist,
    ...
    linear_curr_velocity = np.linalg.norm(
        [current_twist.linear.x, current_twist.linear.y, current_twist.linear.z]
    )
    linear_des_velocity = np.linalg.norm(
        [desired_twist.linear.x, desired_twist.linear.y, desired_twist.linear.z]
    )
    linear = np.fabs(linear_curr_velocity - linear_des_velocity) < linear_tolerance

    angular_curr_velocity = np.linalg.norm(
        [current_twist.angular.x, current_twist.angular.y, current_twist.angular.z]
    )
    angular_des_velocity = np.linalg.norm(
...
def stopped_at_pose(
    current_pose: Pose, desired_pose: Pose, current_twist: Twist
) -> bool:
...
def transform(
    origin_frame: str, dest_frame: str, poseORodom: Union[Pose, PoseStamped, Odometry]
) -> Union[Pose, PoseStamped, Odometry]:
    ...
    trans = tfBuffer.lookup_transform(
        dest_frame, origin_frame, Time(), Duration(seconds=0.5)
    )
    ...
    if isinstance(poseORodom, Odometry):
        ...
        transform_pose_stamped = tf2_geometry_msgs.do_transform_pose_stamped(
            temp_pose_stamped, trans
        )
        ...
        # transforming points
        transformed_twise_point_linear_stamped = tf2_geometry_msgs.do_transform_point(
            twist_point_linear_stamped, trans
        )
        transformed_twise_point_angular_stamped = tf2_geometry_msgs.do_transform_point(
        ...
        transformed_odometry = Odometry(
            header=Header(frame_id=dest_frame),
